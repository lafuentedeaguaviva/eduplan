...
export default function ReportHeader({ initialPdc, fullReportData, viewMode, revisionStatus }: ReportHeaderProps) {
    ...
    return (
        <header className="glass-premium rounded-[1.5rem] md:rounded-[2.5rem] p-6 lg:p-8 border-white/50 shadow-luxe flex flex-wrap xl:flex-nowrap justify-between items-center gap-6 lg:gap-8 sticky top-4 z-50 animate-fade-in-up overflow-hidden">
            ...
            <div className="flex flex-wrap md:flex-row gap-4 w-full xl:w-auto items-center justify-end shrink-0">                <button
                    onClick={() => exportToWord(initialPdc, fullReportData)}
                    className="flex-1 md:flex-none px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 shadow-xl group bg-white hover:bg-slate-50 border border-slate-100 text-slate-700 shadow-blue-900/5 hover:scale-105 active:scale-95"
                >
