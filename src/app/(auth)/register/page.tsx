...
export default function RegisterPage() {
    ...
    const handleGoogleRegister = async () => {
        ...
        try {
            ...
            if (!res.success) {
                setGoogleError(res.error?.message || 'Proveedor no habilitado en Supabase');
                setGoogleLoading(false);
            }
        } catch (err: any) {
            setGoogleError('Error de red al intentar conectar con Google');
            setGoogleLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#0a0c10] relative overflow-hidden font-sans selection:bg-blue-500/30 selection:text-white flex items-center justify-center p-6">
            ...
            <div className="relative z-10 w-full max-w-xl animate-fade-in-up py-12">
                {/* Logo Section */}
                        ...
                        <div className="space-y-8 relative z-10">
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em]">
                                    Únete a la nueva era de la planificación
                                </p>
                            </div>
                            ...
                            <div className="space-y-4">
                                <button
                                    ...
                                    className="group/btn relative w-full h-[64px] bg-white hover:bg-slate-50 text-slate-900 font-black rounded-3xl shadow-xl transition-all duration-500 active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-4 overflow-hidden"
                                >
                                    {googleLoading ? (
                                        <div className="w-5 h-5 border-4 border-slate-200 border-t-blue-600 rounded-full animate-spin"></div>
                                    ) : (
                                        <>
                                ...
                                {googleError && (
                                    <p className="text-[9px] text-rose-400 font-black uppercase text-center tracking-widest animate-shake">
                                        ⚠️ {googleError}
                                    </p>
                                )}
                            </div>
